  ...
  return (
    <>
      ...
      <form onSubmit={add} className="mt-6 card-line bg-cream p-6 space-y-4">
        ...
        <input name="image_url" placeholder="URL da imagem" required className="admin-input" />
        <input name="post_url" placeholder="Link do post no Instagram (opcional)" className="admin-input" />
        <div className="flex gap-4">
          <select name="aspect_ratio" className="admin-input">
            {RATIOS.map((r) => (
              <option key={r} value={r}>{r}</option>
            ))}
          </select>
          <input name="posted_at" type="date" className="admin-input" />

...
export function AdminComments() {
  ...
  return (
    <>
      ...
      <h1 className="font-serif text-3xl text-paper">Comentários</h1>
      <p className="mt-2 text-mist">Moderar comentários dos artigos.</p>
      ...
      {status !== 'loading' && items.length === 0 && (
        <p className="py-16 text-center text-mist">Sem comentários neste filtro.</p>
      )}

      {items.length > 0 && (
        <ul className="mt-6 space-y-4">
          {items.map((c) => (
            <li key={c.id} className="card-line bg-graphite p-5">
              <div className="flex items-baseline justify-between gap-4">
                <p className="font-medium text-paper">{c.author_name}</p>
                <p className="text-xs text-mist">{formatDate(c.created_at)}</p>
              </div>
              <p className="text-xs text-mist">{c.author_email}</p>
              <p className="mt-3 whitespace-pre-line text-sm text-paper/80">{c.body}</p>
...
function ActBtn({
  ...
  const base =
    'border px-3 py-1 text-xs transition-colors disabled:opacity-50 ' +
    {
      ...
      muted: 'border-graphite-light text-mist hover:border-paper',
